...
export async function getPlayers(_req: Request, res: Response): Promise<void> {
  try {
    const result = await pool.query(
      "SELECT player_id, player_name, team, jersey_number, position FROM players ORDER BY player_name",
    );
...
export async function getMatches(_req: Request, res: Response): Promise<void> {
  try {
    const result = await pool.query(
      "SELECT match_id, match_date, stadium, city, tournament_stage FROM matches ORDER BY match_date DESC, match_id",
    );
    res.json(result.rows);
  } catch (error) {
    console.error(error);
