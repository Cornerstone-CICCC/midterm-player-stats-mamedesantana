...
export async function getPerformances(
  ...
  try {
    ...
    const sort =
      sortColumns[String(req.query.sort ?? "match_date")] ??
      sortColumns.match_date;
    const order =
    ...
    if (search) {
      ...
      conditions.push(
        `(p.player_name ILIKE ${p} OR p.team ILIKE ${p} OR p.position ILIKE ${p} OR pf.opponent_team ILIKE ${p})`,
      );
    }
    if (team) {
      values.push(team);
...
export async function getPerformanceById(
  ...
  if (!Number.isInteger(id) || id <= 0) {
    res.status(400).json({ message: "Invalid performance ID" });
    return;
  }
  try {
    const result = await pool.query(
...
export async function createPerformance(
  ...
  try {
    if (!req.body.player_id || !req.body.match_id) {
      res.status(400).json({ message: "player_id and match_id are required" });
      return;
    }
    const columns = editableColumns.filter(
      (column) => req.body[column] !== undefined,
    );
    const values = columns.map((column) =>
      req.body[column] === "" ? null : req.body[column],
    );
    const placeholders = columns.map((_, index) => `$${index + 1}`);
    const result = await pool.query(
      `INSERT INTO performances (${columns.join(", ")}) VALUES (${placeholders.join(", ")}) RETURNING *`,
      values,
    );
    ...
    if (error?.code === "23505") {
      res
        .status(409)
        .json({
          message: "This player already has a performance for this match",
        });
      return;
    }
    if (error?.code === "23503") {
      res.status(400).json({ message: "Invalid player or match" });
      return;
    }
...
export async function updatePerformance(
  ...
  try {
    ...
    if (!columns.length) {
      res.status(400).json({ message: "No fields supplied" });
      return;
    }
    const values = columns.map((column) =>
      req.body[column] === "" ? null : req.body[column],
    ...
    const result = await pool.query(
      `UPDATE performances SET ${sets.join(", ")} WHERE id = $${values.length} RETURNING *`,
      values,
    );
    if (!result.rowCount) {
      res.status(404).json({ message: "Performance not found" });
